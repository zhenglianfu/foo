simp.ready(function(){
	simp.require("dom", function(data){
		var dom = data.dom;
		//begin
		var div = dom("<div>hehe</div>");
	});
});
...
// 3des for nongfu 
function triple_des(key, message){
	key = "" + key;
	message = "" + message;
	message = ( message.length > 10 ? message.length : "0" + message.length ) + message;
	for (var i = message.length; i < 32; i++) {
		message += "0";
	}
	message = string2hex(message);
	key = CryptoJS.MD5(key).toString().toUpperCase();
	key = key + key.substr(0, 16);
	return string2hex(des(string2byte(key), string2byte(message), 1, 0, 0, 1));
}
